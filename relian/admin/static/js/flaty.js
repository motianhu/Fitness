$(function () {
    ...
    //if ( browserName == "Microsoft Internet Explorer" ) window.location="http://order.legou7.com/noie";
    //-->
    ...
    $('#noReadMsgId').popover({container:'body'});
    $('#noReadMsgId').popover('show');
    ...
    if (jQuery().plot) {
        ...
        function i() {
            ...
            $.plot($("#chart_1"), [
                ...
                {label: "cos(x)", data: n},
                {label: "tan(x)", data: r}
            ], {series: {lines: {show: true}, points: {show: true}}, xaxis: {ticks: [0, [Math.PI / 2, "π/2"], [Math.PI, "π"], [Math.PI * 3 / 2, "3π/2"], [Math.PI * 2, "2π"]]}, yaxis: {ticks: 10, min: -2, max: 2}, grid: {backgroundColor: {colors: ["#fff", "#eee"]}}})
        }
        ...
        function s() {
            ...
            var r = $.plot($("#chart_2"), [
                {data: t, label: "Unique Visits"},
                {data: n, label: "Page Views"}
            ], {series: {lines: {show: true, lineWidth: 2, fill: true, fillColor: {colors: [
                {opacity: .05},
                {opacity: .01}
            ]}}, points: {show: true}, shadowSize: 2}, grid: {hoverable: true, clickable: true, tickColor: "#eee", borderWidth: 0}, colors: ["#FCB322", "#A5D16C", "#52e136"], xaxis: {ticks: 11, tickDecimals: 0}, yaxis: {ticks: 11, tickDecimals: 0}});
            var s = null;
        ...
        function o() {
            function o() {
                ...
                for (n = 0; n < u.length; ++n) {
                    ...
                    if (l == null)f = c[1]; else if (c == null)f = l[1]; else f = l[1] + (c[1] - l[1]) * (e.x - l[0]) / (c[0] - l[0]);
            ...
            plot = $.plot($("#chart_3"), [
                ...
                {data: t, label: "cos(x) = -0.00"}
            ], {series: {lines: {show: true}}, crosshair: {mode: "x"}, grid: {hoverable: true, autoHighlight: false}, colors: ["#FCB322", "#A5D16C", "#52e136"], yaxis: {min: -1.2, max: 1.2}});
        ...
        function u() {
            ...
            var e = {series: {shadowSize: 1}, lines: {show: true, lineWidth: .5, fill: true, fillColor: {colors: [
                {opacity: .1},
                {opacity: 1}
            ]}}, yaxis: {min: 0, max: 100, tickFormatter: function (e) {
                return e + "%"
    ...
    if (jQuery().fullCalendar) {
        ...
        $("#calendar").fullCalendar({header: d, editable: true, droppable: true, drop: function (e, t) {
            ...
            {title: "All Day Event", start: new Date(p, h, 1), className: "label label-default"},
            {title: "Long Event", start: new Date(p, h, c - 5), end: new Date(p, h, c - 2), className: "label label-success"},
            {id: 999, title: "Repeating Event", start: new Date(p, h, c - 3, 16, 0), allDay: false, className: "label label-default"},
            {id: 999, title: "Repeating Event", start: new Date(p, h, c + 4, 16, 0), allDay: false, className: "label label-important"},
            {title: "Meeting", start: new Date(p, h, c, 10, 30), allDay: false, className: "label label-info"},
            {title: "Lunch", start: new Date(p, h, c, 12, 0), end: new Date(p, h, c, 14, 0), allDay: false, className: "label label-warning"},
            {title: "Birthday Party", start: new Date(p, h, c + 1, 19, 0), end: new Date(p, h, c + 1, 22, 30), allDay: false, className: "label label-success"},
            {title: "Click for Google", start: new Date(p, h, 28), end: new Date(p, h, 29), url: "http://google.com/", className: "label label-warning"}
        ]});
    ...
    if (jQuery.plot) {
        ...
        $.plot(y, [
            {label: "Visits", data: b, lines: {fillColor: "#f2f7f9"}, points: {fillColor: "#88bbc8"}},
            {label: "Unique Visits", data: w, lines: {fillColor: "#fff8f2"}, points: {fillColor: "#ed7a53"}}
        ], S)
    }
    if (jQuery().sparkline) {
