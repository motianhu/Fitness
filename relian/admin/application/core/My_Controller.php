...
class My_Controller extends CI_Controller
{
	...
	var $_user = '';
	var $menu_list = '';
	var $manage_role_list = array('1','2'); //管理角色id
	...
	public function __construct()
	{
		...
		if( ! $this->menu_list)
		{
			...
			$this->menu_list = $this->menu_model->get_all_menu(array('disabled'=>'false'));
		...
		$this->template->assign ( 'manage_role_list', $this->manage_role_list );
	...
	protected function check_auth()
	{
		...
		if( ! in_array($uriArr3,array('index','save','del')) )
			return true;
		$auth = $this->session->userdata('auth');
		if( empty($auth[$uriArr[1].'/'.$uriArr[2]][$uriArr3]) )
			show_error ( '权限不足');
	}
	...
	{
		$category_list = $this->cache->file->get('category');
		if( ! $category_list)
		{
			$this->load->model('category_model');
			$result = $this->category_model->get_all_category(array('disabled'=>'false'));
			foreach ($result as $v)
				$category_list[$v['cat_id']] = $v['cat_name'];
			$this->cache->file->save('category', $category_list, file_cache_time);
		}
