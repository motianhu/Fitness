...
class Schedule extends My_Controller
{
	...
	{
		$data['store'] = $this->store_model->get_store_list($this->_user['store_id']);
		
		$data['footerJs'] = array('DatePicker/WdatePicker.js','schedule.js');
		$this->template->display ( 'shop/schedule/list.html', $data );
	}
	...
	public function save(){
		...
		for( $i = 1; $i <= 7; $i++){
			...
			if( strtotime($tmp_date) < strtotime(date("Y-m-d",strtotime("+1 day"))) ){
				continue;
			}
		...
		//获取该时间段已选择课程
		$choose_schedule = $this->schedule_model->get_choose_schedule($coach_id,$choose_date,'store_id,coach_id,date,time');
		//比较差异
		...
		redirect ( base_url () . 'shop/schedule/');
	}
	
	//获取分店课程教练列表
	public function get_coach_list($store_id = 0){
		echo json_encode($this->admin_model->get_coach_by_store($store_id));
	}
	//获取分店教练课程列表
	public function get_course_list($coach_id = 0){
		echo json_encode($this->course_model->get_course_by_coach($coach_id));
	}
