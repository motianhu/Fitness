...
class Menu extends My_Controller {
    ...
    public function index($page = 1)
    {
    	...
    	$data ['list'] = $this->menu_model->get(NULL,pageSize,$page);
    	// 分页
    ...
    {
    	//菜单 优先缓存
    	$menu = $this->cache->file->get('param');
    	if( ! $menu)
    	{
    		$menu = $this->menu_model->get_all_menu(array('disabled'=>'false'));
    ...
    public function del($id)
    {
    	if($id)
    	{
	    	$this->menu_model->del(array('menu_id'=>(int)$id));
	    	redirect(base_url().'sys/menu/index');
    	}
    	else
    	{
